...
public class MargaritaFirefoxProfile extends FirefoxProfile {
     ...
     */
    private static final ImmutableMap<String, Object> FROZEN_PREFERENCES = ImmutableMap
     ...
     */
    public MargaritaFirefoxProfile(File profileDir) {
	model = profileDir;
	verifyModel(model);
	...
	if (prefsInModel.exists()) {
	    ...
	    enableNativeEvents = getBooleanPreference(
		existingPrefs,
		ENABLE_NATIVE_EVENTS_PREF,
		DEFAULT_ENABLE_NATIVE_EVENTS);
	    acceptUntrustedCerts = getBooleanPreference(
		existingPrefs,
		ACCEPT_UNTRUSTED_CERTS_PREF,
		ACCEPT_UNTRUSTED_CERTIFICATES);
	    untrustedCertIssuer = getBooleanPreference(
		existingPrefs,
	...
	if (!model.isDirectory()) {
	    throw new UnableToCreateProfileException("Given model profile directory is not a directory: "
    ...
    /**
     * Set a preference for this particular profile. The value will be properly
     * quoted before use. Note that if a value looks as if it is a quoted string
     * (that is, starts with a quote character and ends with one too) an
     * IllegalArgumentException is thrown: Firefox fails to start properly when
     * some values are set to this.
     * 
     * @param key
     *            The key
     * @param value
	...
	checkNotNull(value);
	checkArgument(
	    !FROZEN_PREFERENCES.containsKey(key)
	...
	// set to 3 to restore previous session!
	prefs.setPreference("browser.startup.page", 3);
     ...
     * signed by an unknown authority or are generally untrusted. This is set to
     * true by default.
     * 
    ...
    /**
     * By default, when accepting untrusted SSL certificates, assume that these
     * certificates will come from an untrusted issuer or will be self signed.
     * Due to limitation within Firefox, it is easy to find out if the
     * certificate has expired or does not match the host it was served for, but
     * hard to find out if the issuer of the certificate is untrusted.
     * <p/>
     * By default, it is assumed that the certificates were not be issued from a
     * trusted CA.
     * <p/>
     * If you are receive an "untrusted site" prompt on Firefox when using a
     * certificate that was issued by valid issuer, but has expired or is being
     * served served for a different host (e.g. production certificate served in
     ...
     * directory is returned. Note that this profile directory is a temporary
     * one and will be deleted when the JVM exists (at the latest)
     * 
     * This method should be called immediately before starting to use the
     * profile and should only be called once per instance of the
